...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Registrar dinámicamente el receiver con un IntentFilter
        IntentFilter filter = new IntentFilter();
        filter.addAction("com.example.broadcastrecordatoriosimple.ACTION_REMINDER");

        // Aquí se especifica si el receiver es exportado o no (para evitar el error de seguridad)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            registerReceiver(reminderReceiver, filter, Context.RECEIVER_EXPORTED);
        } else {
    ...
    private void scheduleExactAlarm(AlarmManager alarmManager) {
        Intent intent = new Intent("com.example.broadcastrecordatoriosimple.ACTION_REMINDER");
